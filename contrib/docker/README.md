...
1. Build the base image with a tag to make it easy to reference later.  These
   ...
   **IMPORTANT: This MUST be run from the main directory of the dcrd code repo.**
   ...
   To build a specific `git` tag or branch, the `VAR_BUILD_TAG` argument can be specified.  The argument is optional and defaults to `master`.

   ```
    $ docker build --build-arg VAR_BUILD_TAG=release-v1.x.x \
...
3. Run `dcrd` on `mainnet` in the background using the aforementioned data
   ...
   $ docker run -d --read-only \
     ...
     -p ${VAR_MAINNET_P2P_PORT}:${VAR_MAINNET_P2P_PORT} \
...
```sh
$ docker stop -t 60 "${VAR_CONTAINER_NAME}"
$ docker start "${VAR_CONTAINER_NAME}"
```
...
- `MONETARIUM_ALT_DNSNAMES` (Default: None):  
...
`--altdnsnames` CLI parameter or the `MONETARIUM_ALT_DNSNAMES` environment variable to
...
# to localhost.
$ VAR_MAINNET_RPC_PORT=9109
$ docker run -d --read-only \
  --name "${VAR_CONTAINER_NAME}" \
  -v decred-data:/home/decred \
  -p ${VAR_MAINNET_P2P_PORT}:${VAR_MAINNET_P2P_PORT} \
  -p 127.0.0.1:${VAR_MAINNET_RPC_PORT}:${VAR_MAINNET_RPC_PORT} \
  "${VAR_IMAGE_NAME}" --altdnsnames "${VAR_CONTAINER_NAME}"
